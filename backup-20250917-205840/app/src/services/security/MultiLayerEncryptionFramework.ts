...
export class MultiLayerEncryptionFramework {
  ...
  async rotateTierKeys(
    ...
    try {
      ...
      for (const layer of layersToRotate) {
        try {
          const keyName = `@fullmind_ml_${layer}_key_${subscriptionTier}_v2`;
          const rotationDateKey = `@fullmind_ml_${layer}_rotation_${subscriptionTier}`;
  ...
  private async getKeyRotationInfo(
    ...
    try {
      const rotationDateKey = `@fullmind_ml_${layer}_rotation_${tier}`;
