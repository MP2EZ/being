/**
 * FullMind Widget Config Plugin
 * Expo config plugin for iOS and Android widgets in FullMind MBCT app
...
const withIOSWidget = (config) => {
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      const widgetTargetPath = path.join(iosProjectPath, 'FullMindWidget');
...
const withIOSEntitlements = (config) => {
  return withEntitlementsPlist(config, (config) => {
    const appGroupId = 'group.com.fullmind.mbct.widgets';
...
const withAndroidManifest = (config) => {
  return withAndroidManifest(config, (config) => {
    ...
    const widgetReceiver = {
      $: {
        'android:name': '.widget.FullMindWidgetProvider',
...
async function updateXcodeProject(iosProjectPath, config) {
  ...
  const projectName = config.name || 'FullMind';
  const bundleIdentifier = config.ios?.bundleIdentifier || 'com.fullmind.mbct';
  
  console.log('\n📝 Manual Xcode Configuration Required:');
  console.log('1. Open your project in Xcode');
  console.log(`2. Add a new Widget Extension target named "FullMindWidget"`);
  console.log(`3. Set bundle identifier to: ${bundleIdentifier}.FullMindWidget`);
  console.log('4. Add the App Group capability to both main app and widget targets');
  console.log('5. Configure App Group ID: group.com.fullmind.mbct.widgets');
