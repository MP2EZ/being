  ...
  try {
    ...
    for (const key of crisisKeys) {
      try {
        ...
        logError(`Failed to migrate key: ${key}`, { error }, LogCategory.Crisis);
      }
    }
...
export async function isCrisisMigrationNeeded(): Promise<boolean> {
  try {
    ...
    logError('Failed to check migration status', { error }, LogCategory.Crisis);
    return false;
  }
}
...
export async function getCrisisMigrationStatus(): Promise<{
  ...
  try {
    ...
    logError('Failed to get migration status', { error }, LogCategory.Crisis);
    return {
      asyncStorageKeys: 0,
      secureStoreKeys: -1,
