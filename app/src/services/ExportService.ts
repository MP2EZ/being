...
class ExportService {
  ...
  private async generatePDF(data: any, options: ExportOptions): Promise<ExportResult> {
    try {
      ...
      const fileName = `fullmind-report-${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  private async generateCSV(data: any, options: ExportOptions): Promise<ExportResult> {
    try {
      ...
      // Save CSV file
      const fileName = `fullmind-data-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  private generatePDFTemplate(data: any, options: ExportOptions): string {
    ...
    <body>
      <div class="header">
        <h1>FullMind Wellness Report</h1>
      ...
      <div class="summary">
        ...
        <p>This report contains your FullMind wellness data for the specified period. 
      ...
      <div class="footer">
        <p>Generated by FullMind - Mindfulness-Based Cognitive Therapy App</p>
