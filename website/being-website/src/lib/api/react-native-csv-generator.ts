/**
 * Being. React Native CSV Generator - Research-Ready Data Export
 * 
...
export class ReactNativeCSVGenerator implements ClinicalCSVGenerator {
  ...
  async generateResearchDataCSV(
    ...
    try {
      ...
      // Write to file
      const fileName = `being-research-export-${Date.now()}.csv`;
  ...
  async generateTimeSeriesCSV(
    ...
    try {
      ...
      // Write to file
      const fileName = `being-timeseries-export-${Date.now()}.csv`;
      const filePath = `${this.tempDirectory}/${fileName}`;
  ...
  async generateMBCTProgressCSV(
    ...
    try {
      ...
      // Write to file
      const fileName = `being-mbct-progress-${Date.now()}.csv`;
      const filePath = `${this.tempDirectory}/${fileName}`;
  ...
  async generateAggregatedMetricsCSV(
    ...
    try {
      ...
      // Write to file
      const fileName = `being-metrics-export-${Date.now()}.csv`;
      const filePath = `${this.tempDirectory}/${fileName}`;
