-- Baseline migration: full app schema (INFRA — Supabase prod provisioning).
--
-- The production project (yliycxslzdsgjtpxggtf) was never provisioned beyond
-- 20260522000000_webhook_replay_cache — users/encrypted_backups/analytics_events/
-- subscriptions and all RLS/functions/views were absent, so crisis_detected
-- telemetry, Cloud Backup, and subscriptions had no tables to land in.
--
-- This baselines app/src/core/services/supabase/schema.sql (the canonical full
-- schema, incl. the FEAT-129 §6b crisis views) into the migration history so
-- 'supabase db push' can stand up a fresh environment. Ordered after the webhook
-- migration and before 20260605000000 (crisis views), which then becomes an
-- idempotent CREATE OR REPLACE no-op on top. Verbatim copy of schema.sql.
-- ======================================================================
