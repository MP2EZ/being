...
- **Passive staleness ≠ true liveness.** A server-side cron cannot prove the *on-device*
  detection→Supabase pipeline is alive — only that recent rows exist. At low volume it cannot
  perfectly separate "dead pipeline" from "genuinely quiet." The manual active-liveness
  assertion (step 1) remains the gold standard until the **synthetic end-to-end probe** follow-up lands.
...
- **Synthetic end-to-end liveness probe.** A scheduled probe that exercises the real detection
  emit path so liveness can distinguish "dead" from "quiet" at low volume. _(Tracked: follow-up.)_
