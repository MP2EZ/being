...
2. **`session_id` does not actually rotate within a process.** It is minted once in the `SupabaseService` constructor and never re-generated, and its date component is derived from a **UTC** day boundary. A session spanning midnight UTC therefore carries the previous day's prefix. The daily-rotation claim holds only across app restarts.
