...
- `audit_log_<epoch>` — written to **AsyncStorage** by `SecureStorageService.logStorageAccess` on every `crisis_tier` operation and every failure. It carries storage-access metadata (operation type, storage key, tier, sensitivity level, timing), not wellness content, but it records that a crisis-tier record existed and when. This one was unerasable only **by omission**: the keys enumerate, and `audit_log_` was simply absent from the erasure sweep. It has been added, which reaches records already written on shipped installs with no migration.
...
The recurring failure mode is omission, so this section now carries a **positive enumeration** (below) instead of a fourth per-defect assurance. A negative claim of the form "nothing else remains" has a 0-for-3 record here and is not independently checkable; a list of every persisted key on the crisis path is. The AsyncStorage half of that list is machine-verified: `crisisRecordErasure.privacy.test.ts` reads the same `SWEPT_ASYNC_PREFIXES` / `SWEPT_EXACT_KEYS` constants production filters on and asserts nothing escapes them.

**Every persisted key on the crisis path.**

| Key / pattern | Store | Written by | Encrypted? | Erasure mechanism |
|---|---|---|---|---|
| `crisis_async_<id>` | AsyncStorage | `storeCrisisData` | AES-256-GCM | `SWEPT_ASYNC_PREFIXES` |
...
| `storage_metadata_index` | AsyncStorage | `storeMetadata` / `deleteMetadata` | **no** | `SWEPT_EXACT_KEYS` **+ cache clear** (control 19) |
...
**Residual this does not close.** DEBUG-381 makes `storage_metadata_index` *erasable*; it does not make it *confidential*. It remains an unencrypted AsyncStorage record naming crisis episode ids, their tier, their sensitivity level and their access timing, present for the life of the install until erasure. Encrypting or minimising it is a separate decision, not taken here.
...
| 18 | **Erasure coverage for storage-access audit logs (DEBUG-355).** `logStorageAccess` persists an entry under `audit_log_<epoch>` on every `crisis_tier` operation and every failure. That prefix is now part of `SWEPT_ASYNC_PREFIXES`, the single constant `clearAllWellnessData` filters on and the erasure privacy suites assert against — replacing the hand-copied prefix mirrors that previously let production and test drift apart unnoticed. Because these keys enumerate, the control reaches records already written on shipped installs; no sweeper entry or persisted index is required. | `app/src/core/services/security/SecureStorageService.ts`; `app/src/core/services/security/__tests__/auditLogErasure.privacy.test.ts` | `SWEPT_ASYNC_PREFIXES` / `clearAllWellnessData` |
| 19 | **Erasure coverage for the storage metadata index (DEBUG-381).** `storage_metadata_index` is an unencrypted AsyncStorage key holding one record per stored blob — `storageKey` (e.g. `crisis_async_<episodeId>`), `storageTier: crisis_tier`, `sensitivityLevel: level_1_crisis_responses`, `dataType`, and access timestamps. It contains no wellness content, but establishes that a crisis episode existed, when, and how often it was read. It matched no swept prefix and survived account deletion. **The control is the ORDERING, not merely the sweep entry, and crediting only the latter would be an affirmatively false control claim.** `storeMetadata` re-serialises the entire in-memory `metadataCache` on every write and the sweep did not clear that cache, so adding the key to `SWEPT_EXACT_KEYS` alone would delete the file and let the next crisis or assessment write restore it verbatim, erased records included. The fix clears the cache **synchronously, before the sweep's first `await`** — any later position reopens a window in which an in-flight write captures the pre-erasure map — and applies unconditionally on both the account-deletion and logout branches. Because the key is a single fixed AsyncStorage name, the control reaches records already written on shipped installs, with no migration and no sweeper entry. Pinned by a regression test that asserts the erased episode id does not reappear after a *subsequent* write; the sweep-only non-fix passes every other assertion in that suite and fails exactly that one. | `app/src/core/services/security/SecureStorageService.ts`; `app/src/core/services/security/__tests__/storageMetadataIndexErasure.privacy.test.ts` | `metadataCache.clear()` + `SWEPT_EXACT_KEYS` / `clearAllWellnessData` |
...
| 2.0 | 2026-08-08 | Palouse Labs LLC | DEBUG-381: `storage_metadata_index` — a fourth local crisis-path survivor — added to the erasure sweep, with the in-memory `metadataCache` cleared ahead of it to close a write-back loop that would otherwise have made the sweep entry inert. §3 restructured: the recurring per-defect completeness note is **replaced by a positive enumeration of every persisted key on the crisis path**, because the negative form ("nothing else remains") had been asserted and falsified in three consecutive versions — v1.8, v1.9, and this one — each time discovered by enumerating the store after erasure rather than by review. §7 gains control 19. Version bumped to 2.0 for the structural §3 change, not for a change in processing. |
