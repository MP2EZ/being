...
- **Vocabulary ruling, stated because the delta above invites the question (FEAT-588):** a tester who runs the standalone practices reported the loop's breath as missing something those have. `philosopher` and `ux` ruled independently and agreed: the beat is a **gate wearing session chrome**, not a diminished practice, and the differences are intent. It starts itself (no Begin), carries no name while it runs, ends in the next beat rather than a completion screen, has no haptics at all — neither breath cues nor session anchors — because the opt-in promises eyes-closed practice and this beat is eyes-open by construction, and is absent from the practice library. The loop is the practice; this is its threshold. Recorded so the same report is answered from the record — the full ruling, including what was considered and declined, is in `DailyLoopStepScreen`'s module header.
- **Technique delta, also stated (MAINT-391):** the app **paces** this breath — 4-4 today, and the engine can pace an asymmetric extended exhale (e.g. 4-6) — but it **does not offer breath retention**, and after MAINT-391 it cannot: the hold was deleted from the pattern type and from `BreathingCircle`. Note first that the framework asks for neither. This element says "bring attention to physical sensations of breathing"; it prescribes **no ratio and no retention**, so pacing at all is already the product being more prescriptive than the source, and retention would be a second step further with nothing in the framework calling for it. Note second **why it is not a small step**: retention is not classical Stoic technique — no *prosochē* passage in Epictetus, Marcus, or Seneca describes controlling the breath, and the Stoic breath (*pneuma*) is cosmology, not a practice instruction. It could only enter this document under the **[Aware Presence](../principles/01-aware-presence.md)** limb, i.e. as borrowed contemplative-mindfulness technique, and would have to be **labelled as such** rather than presented as Stoic. And it could only enter the product after a **safety pass**: retention is a recognised CO2 / interoceptive panic provocation, and Being's audience includes users screening at high GAD-7 — so a `crisis` agent review is a precondition, not a courtesy. The engineering ruling and the reintroduction bar live in `app/src/features/practices/shared/breathingPatterns.ts`.
