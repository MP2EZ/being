...
npm run e2e:safety                 # the 14 sim-runnable safety flows (a jest tripwire pins the count)
...
- **A physical iPhone whose tunnel is asleep reads as “no device attached.”** `e2e_attached_devices` drops `tunnelState: "disconnected"` (`e2e-real-device.sh:35`) and the tunnel is lazy, so a wired, paired, Developer-Mode-enabled iPhone still enumerates EMPTY at exit 0 — which `e2e_resolve_real_device` refuses on, reporting no device while you are holding one. Distinguish the two rejected states before reacting: `disconnected` with `transport: wired` is a sleeping tunnel — wake it with `xcrun devicectl device info details --device <udid>` and re-resolve; `unavailable` with `transport: None` is genuinely unplugged. **But device flows cannot run at all right now (DEBUG-589)** — maestro 2.6.0 fails to build its XCUITest driver for a connected iPhone, so `e2e:safety:988-dial` dies in ~8s with `NO_REPORT` before any step runs. Read that as a toolchain failure, never as a dial-path regression. A freshly-woken tunnel can also refuse `process launch` for an untrusted profile until it revalidates; re-wake and retry before sending anyone to Settings. Matters most on `npm run e2e:safety:988-dial`, the only device-only flow and the one pinning the crisis dial path.
