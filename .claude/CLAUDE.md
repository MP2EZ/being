...
- Maestro safety-e2e (INFRA-171) is **local-only** — no CI macOS runners. Install with `brew install mobile-dev-inc/tap/maestro` (NOT `brew install maestro` — that's a different cask; see `docs/testing/e2e-maestro.md`). Prereq per run: an iOS sim must be booted with a **no-dev-client** build of `fyi.being.app` installed — run **`npm run e2e:safety:build`** (EAS local `e2e-sim` build + install) once per worktree, **not `npm run ios`** (INFRA-216). **That build needs a clean working tree** — `eas.json` sets `requireCommit:true`, so commit or stash first; since INFRA-329 the script pre-flights `git status --porcelain` and fails in milliseconds naming the offending paths, instead of letting EAS abort ~30s in. There is no bypass flag by design (the script is the gate's only evidence the build under test is fresh — DEBUG-315). Related: **never pipe the build command** (`| tee`, `| tail`) — the pipeline reports the *last* command's status, so a failed build reads as exit 0; use `set -o pipefail` if you must capture output. The gate target must EXCLUDE `expo-dev-client`: it's a project dependency, so it links into both `npm run ios` AND a plain `expo run:ios --configuration Release` — both still show the Expo dev launcher, which the flow can only navigate by a guessed-coordinate tap, so the gate flakes badly (INFRA-216: every sim flow hit 0–1/5 on a dev build AND on plain Release). Only the EAS `e2e-sim` profile (`developmentClient:false`) removes the launcher; the no-dev-client build boots straight to the app and is effectively what TestFlight/users run. Prereqs: `eas-cli` logged in + `fastlane` (`brew install fastlane`; if it then breaks `pod`, `brew reinstall cocoapods`). **Known limitation (INFRA-216 follow-up):** the no-dev-client build removes the launcher but the slower Release build leaves the long onboarding preamble timing-fragile (not yet ≥5/5 consecutive) — seed-state preamble-skip is the tracked follow-up; expect occasional retries until then. Flows live in `app/.maestro/`. The `/b-close` Phase 2.5 gate runs scoped flows when safety paths change. `--skip-e2e` bypass mirrors the `--no-verify` policy below: **`hotfix/*` only** — refused on `feat/*`, `fix/*`, `chore/*`. Authoring + debugging guide: `docs/testing/e2e-maestro.md`.
