...
npm run e2e:safety                 # the 9 sim-runnable safety flows (nothing enforces this count)
...
| Breathing 60fps | **Structural proxy only — the frame rate is still unmeasured.** INFRA-306 shipped `npm run check:breathing-worklets` (CI, `Performance regression` job): it fails if the PERF-01/PERF-02 shape returns to the animation path — `runOnJS`/state-setter inside a `useAnimatedStyle`/`useDerivedValue`/`useAnimatedReaction`/`useFrameCallback` body, `requestAnimationFrame` on that path, or `BreathingCircle` losing `React.memo` / its module-scope prop constants. It does **not** measure frames and cannot: CI is 100% `ubuntu-latest`. Real on-device measurement is INFRA-373, blocked on TWO things: naming a calibration handset, AND respecifying its ACs — they name 6 mount screens when only 3 are live (`PracticeTimerScreen`, `DailyLoopStepScreen`, `DailyLoopCompleteScreen`), and justify themselves by a hold branch no caller reaches (every live caller passes a hold-free 4-4 pattern; the only hold pattern ever authored, the evening 4-4-6, was retired with its flow in FEAT-298 slice 6c). They also targeted `SharedBreathingScreen.tsx`, which **MAINT-386 deleted** — so respecify against `BreathingCircle` itself, which is now the sole entry in `check-breathing-worklets`' guarded set and where MAINT-386 ported the reduce-motion branch that INFRA-373's ACs wanted as a control condition. Note the budget is also device-naive as written — ProMotion nominal is 8.3ms, not 16.67ms — see `post-launch-monitoring-runbook.md` §5a |
