...
|---|---|---|---|
| 0 | **Safety-adjacent tooling.** TYPE = `INFRA`/`MAINT` **AND** Name matches `gate`, `harness`, `worktree`, `simulator`, `DerivedData`, `lease`, `CI`, `build` **AND** Technical Notes reference no protected path | `quick` | Tooling that protects safety code is not safety code. `DEBUG` is excluded deliberately: Tier 3 already routes it to `quick`, so admitting it here could only pre-empt Tier 1 — a downgrade for the common case of a crisis-behaviour bug whose name happens to contain `build` or `CI`. |
